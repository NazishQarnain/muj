  ...
  $("#app").innerHTML = `
    <div class="max-w-md mx-auto p-6 border rounded-2xl dark:border-zinc-800">
      ...
      <form id="loginForm" class="space-y-3">
        ...
        <p class="text-right text-xs text-zinc-400 -mt-1"><span class="underline cursor-pointer hover:text-zinc-600" id="forgotLink">Forgot password?</span></p>
    ...
    try {
      ...
      if (!user.emailVerified) {
        ...
        document.getElementById('resendBtn').onclick = async () => {
          // Re-signin temporarily to send email, then sign out again
...
function setupAuthListener() {
  firebase.auth().onAuthStateChanged(async (user) => {
    if (user) {
      ...
      // Sync all Firebase data: photoURL + locked batch
      try {
        const snap = await firebase.database().ref('users/' + user.uid).once('value');
        const data = snap.val() || {};

        if (data.photoURL && !p.photoURL) {
          p.photoURL = data.photoURL; changed = true;
        }
        // Restore locked batch from Firebase (important if user clears localStorage)
