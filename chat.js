...
// Cache for user avatars so we don't hit Firebase on every message
const avatarCache = {};
...
function sendMessage() {
  ...
  // Security check: user can only send in their own locked batch room
  const expectedRoom = buildRoomKey(p.program, p.course, p.batch);
  if (currentRoomKey !== expectedRoom) {
    alert('You can only send messages in your own batch room.');
    return;
  }
...
async function displayMessage(msgData) {
  const chatBox = $("#chatBox");
  if (!chatBox) return;
  ...
  // Fetch avatar — own photo from local profile, others from Firebase
  let photoURL = null;
  if (isOwn) {
    photoURL = getProfile().photoURL || null;
  } else if (msgData.uid) {
    photoURL = await getUserPhoto(msgData.uid);
  }
  const initials = (msgData.displayName || 'S').split(' ').map(w => w[0]).join('').slice(0, 2).toUpperCase();
  const avatarHtml = photoURL
    ? `<img src="${photoURL}" class="w-7 h-7 rounded-full object-cover flex-shrink-0 ${isOwn ? 'order-2' : ''}" />`
    : `<div class="w-7 h-7 rounded-full bg-blue-100 dark:bg-blue-800 flex items-center justify-center text-[10px] font-bold text-blue-700 dark:text-blue-200 flex-shrink-0 ${isOwn ? 'order-2' : ''}">${initials}</div>`;
