...
// Generate year options based on program duration (e.g. "1st Year", "2nd Year")
function getYearsForProgram(programKey) {
  const prog = MUJ_PROGRAMS[programKey];
  if (!prog) return [];
  const labels = ["1st Year","2nd Year","3rd Year","4th Year","5th Year"];
  return labels.slice(0, prog.duration);
}

// Build a unique chat room key from program + course + year
function buildRoomKey(program, course, year) {
  return [program, course, year]
    .join("_")
...
function getProfile() {
  ...
  return raw
    ? JSON.parse(raw)
    : { displayName: "Student", email: "", program: "", course: "", year: "" };
}
